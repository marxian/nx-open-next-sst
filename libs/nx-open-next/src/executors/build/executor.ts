...
export default async function openNextBuildExecutor(
  ...
  try {
    ...
    const packageJson =
      options.packageJson || `${options.sourceRoot}/package.json`;
    const openNextBuildCommand = `open-next build --build-command "${buildCommand}" --build-output-path "${buildOptions.outputPath}" --app-path "${options.sourceRoot}" --package-json "${packageJson}"`;
